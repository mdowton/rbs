				...
				<?php
						if ( get_option( 'woocommerce_review_rating_verification_label' ) === 'yes' ) {
							if ( true || woocommerce_customer_bought_product( $GLOBALS['comment']->comment_author_email, $GLOBALS['comment']->user_id, $post->ID ) ) {
								echo '<em class="verified">(' . __( 'verified owner', 'woocommerce' ) . ')</em> '; // xss ok
							}
						}

					?>
				<h6 title="<?php echo get_comment_date( 'c' ); ?>" class="comment-time"><?php echo get_comment_date( __( get_option( 'date_format' ), 'woocommerce' ) );  // xss ok ?></h6>
				<?php edit_comment_link( sprintf( '[%s]', __( 'Edit', 'construction' ) ) ) ?>
