...
class WPV_Sitemap {
	...
	private function categories($atts){
	  ...
	  'show_feed' => true,
		...
		$feed = ($show_feed === true || $show_feed == 'true') ? __( 'RSS', 'construction' ) : '';
	...
	private function display($query, $show_comment = false) {
		...
		while ( $archive_query->have_posts() ) {
			...
			if ( !empty($title) ) {
				$output .=
					"<li>
						<a href='" . get_permalink() . "' rel='bookmark' title='" . esc_attr( sprintf( __( 'Permanent Link to %s', 'construction' ), $title ) )."'>".
							strip_tags( $title ) . // xss ok
						"</a>".
						($show_comment?" (".get_comments_number().")":""); // xss ok
