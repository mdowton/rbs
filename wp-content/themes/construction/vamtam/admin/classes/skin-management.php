...
class WpvSkinManagement extends WpvAjax {
	...
	public function available_skins() {
		...
		if ( isset( $_POST['prefix'] ) ) {
			...
			echo '<option value="">' . __( 'Available skins', 'construction' ) . '</option>'; // xss ok
	...
	public function load_skin() {
	  ...
	  array(
	  'data' => $data,
	  'text' => '<span class="success">' . __( 'The skin has been imported successfully. Have fun!', 'construction' ) . '</span>',
	  )
		); // xss ok
	...
	public function delete_skin() {
		...
		echo '<span class="error">'. __( 'Cannot delete file.', 'construction' ) . '</span>'; // xss ok
		exit;
	}
