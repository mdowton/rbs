...
if ( !class_exists( "PixelentityThemeUpdate" ) ) {
	class PixelentityThemeUpdate {
		...
		public function check($updates) {
			...
			$this->authors = array_map( 'strtolower', $this->authors );

			foreach ( $installed as $theme ) {
				if ($this->authors && !in_array( strtolower( $theme->{'Author Name'} ), $this->authors )) continue;
				$filtered[htmlspecialchars_decode( $theme->Name )] = $theme;
			}

			foreach ( $purchased as &$theme ) {
				$theme->theme_name = htmlspecialchars_decode( $theme->theme_name );
			}
			unset($theme);
